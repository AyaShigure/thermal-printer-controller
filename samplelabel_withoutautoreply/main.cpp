...
int main(int argc, char *argv[])
{
    ...
    } else if (strstr(port_name, "/dev/lp")) {
        h = CP_Port_OpenLpt(port_name);
    } else if (strstr(port_name, "/dev/tty")) {
        h = CP_Port_OpenCom(port_name, 115200, CP_ComDataBits_8, CP_ComParity_NoParity, CP_ComStopBits_One, CP_ComFlowControl_XonXoff, 0);
    } else if (strstr(port_name, ".")) {
        h = CP_Port_OpenTcp(0, port_name, 9100, 5000, 0);
    }
    if (h == 0) {
